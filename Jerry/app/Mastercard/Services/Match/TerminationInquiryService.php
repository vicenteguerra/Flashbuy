...
class TerminationInquiryService extends Connector {
    ...
    public function getTerminationInquiry(TerminationInquiryRequestType $request, TerminationInquiryRequestOptions $options){
        $response = $this->doSimpleRequest($this->getURL($options->getPageOffset(),$options->getPageLength()), Connector::POST, $this->generateSimpleXML($request));
    ...
    private function generateSimpleXML(TerminationInquiryRequestType $request){
        $xml = null;

        $xml  = "<ns2:TerminationInquiryRequest xmlns:ns2=\"http://mastercard.com/termination\">";
        $xml .=     "<AcquirerId>".$request->getAcquirerId()."</AcquirerId>";
        $xml .=     "<TransactionReferenceNumber>".$request->getTransactionReferenceNumber()."</TransactionReferenceNumber>";
        $xml .=     "<Merchant>";
        $xml .=         "<Name>".$request->getMerchantType()->getName()."</Name>";
        $xml .=         "<DoingBusinessAsName>".$request->getMerchantType()->getDoingBusinessAsName()."</DoingBusinessAsName>";
        $xml .=         "<PhoneNumber>".$request->getMerchantType()->getPhoneNumber()."</PhoneNumber>";
        $xml .=         "<Address>";
        $xml .=             "<Line1>".$request->getMerchantType()->getAddress()->getLine1()."</Line1>";
        $xml .=             "<Line2>".$request->getMerchantType()->getAddress()->getLine2()."</Line2>";
        $xml .=             "<City>".$request->getMerchantType()->getAddress()->getCity()."</City>";
        $xml .=             "<CountrySubdivision>".$request->getMerchantType()->getAddress()->getCountrySubdivision()."</CountrySubdivision>";
        $xml .=             "<PostalCode>".$request->getMerchantType()->getAddress()->getPostalCode()."</PostalCode>";
        $xml .=             "<Country>".$request->getMerchantType()->getAddress()->getCountry()."</Country>";
        $xml .=         "</Address>";
        $xml .=         "<CountrySubdivisionTaxId>".$request->getMerchantType()->getCountrySubdivisionTaxId()."</CountrySubdivisionTaxId>";
        $xml .=         "<NationalTaxId>".$request->getMerchantType()->getNationalTaxId()."</NationalTaxId>";
        $xml .=         "<Principal>";
        $xml .=             "<FirstName>".$request->getMerchantType()->getPrincipal()->getFirstName()."</FirstName>";
        $xml .=             "<LastName>".$request->getMerchantType()->getPrincipal()->getLastName()."</LastName>";
        $xml .=             "<NationalId>".$request->getMerchantType()->getPrincipal()->getNationalId()."</NationalId>";
        $xml .=             "<PhoneNumber>".$request->getMerchantType()->getPrincipal()->getPhoneNumber()."</PhoneNumber>";
        $xml .=             "<Address>";
        $xml .=                 "<Line1>".$request->getMerchantType()->getPrincipal()->getAddress()->getLine1()."</Line1>";
        $xml .=                 "<City>".$request->getMerchantType()->getPrincipal()->getAddress()->getCity()."</City>";
        $xml .=                 "<CountrySubdivision>".$request->getMerchantType()->getPrincipal()->getAddress()->getCountrySubdivision()."</CountrySubdivision>";
        $xml .=                 "<PostalCode>".$request->getMerchantType()->getPrincipal()->getAddress()->getPostalCode()."</PostalCode>";
        $xml .=                 "<Country>".$request->getMerchantType()->getPrincipal()->getAddress()->getCountry()."</Country>";
        $xml .=             "</Address>";
        $xml .=             "<DriversLicense>";
        $xml .=                 "<Number>".$request->getMerchantType()->getPrincipal()->getDriversLicense()->getNumber()."</Number>";
        $xml .=                 "<CountrySubdivision>".$request->getMerchantType()->getPrincipal()->getDriversLicense()->getCountrySubdivision()."</CountrySubdivision>";
        $xml .=                 "<Country>".$request->getMerchantType()->getPrincipal()->getDriversLicense()->getCountry()."</Country>";
        $xml .=             "</DriversLicense>";
        $xml .=         "</Principal>";
        $xml .=     "</Merchant>";
        $xml .= "</ns2:TerminationInquiryRequest>";
